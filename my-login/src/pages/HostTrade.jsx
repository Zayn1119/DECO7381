...
export default function HostTrade({ onBack, onOpenPlanner }) {
  const [meetingId, setMeetingId] = useState("");

  // 进入页面时自动生成（或读取已有）Trade 会议ID，并显示在输入框中
  useEffect(() => {
    ...
    <div className="conf-app">
      <header className="conf-header">
        <div className="conf-title">
          ...
          <div className="conf-muted">
            A single trade show meeting is maintained. Share the ID with exhibitors/attendees.
          </div>
        </div>
      </header>
      ...
      <div className="conf-body" style={{ display: "block" }}>
        ...
        >
          ...
          <button
            className="conf-btn block"
            onClick={() => { /* TODO: sign out if you add auth here */ }}
          ...
          >
            Tip: This ID is created automatically and kept as the only active trade show.
          </div>
        </div>
      </div>
    </div>
